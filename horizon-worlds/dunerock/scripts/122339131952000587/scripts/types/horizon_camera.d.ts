 ...
 *
 * `horizontalTranslationInertiaRate` - How quickly the camera follows the target along XZ plane
 * null - The camera follows the target instantly
 * 0 - the camera doesn't follow the target
 * 1 - the camera follows the target with high delay
 * 5 - noticeable delay (~1 meter difference at default character speed)
 * 10 - minimal delay
 *
 * `verticalTranslationInertiaRate` - How quickly the camera follows the target along Y axis plane
 * null - The camera follows the target instantly
 * 0 - the camera doesn't follow the target height changes
 * 1 - the camera barely follows character vertical movement, the character substantially moves on the screen while jumping
 * 5 - the camera repeats most of the character trajectory while jumping, but with a delay
 * 10 - the character slightly moves along the camera during jump
 *
 * `horizontalRotationRate` - Controls how quickly the camera rotates horizontally to return behind the player avatar.
 * null - default rotation rate applied (1)
 * 0 - the camera does not rotate after the target
 * 1 - default value, relatively slow rotating camera
 * 5 - high speed change
 * 10 - nearly instant rotation
 *
 * `verticalRotationRate` - Controls how quickly the camera rotates vertically to the desired target pitch
 * null - default rotation rate applied (1)
 * 0 - the camera does not change the pitch
 * 1 - default value, relatively slow rotation
 * 5 - high speed change
 * 10 - nearly instant rotation
 *
...
export declare type FollowCameraOptions = {
    ...
    /**
     * @deprecated use horizontalRotationRate and verticalRotationRate instead
     */
    rotationSpeed?: number;
    /**
     * @deprecated use horizontalTranslationInertiaRate and verticalTranslationInertiaRate instead
     */
...
export declare class FollowCameraMode implements ICameraMode {
    ...
    verticalOffset: HorizonProperty<number>;
    /**
     * How quickly the camera follows the target along XZ plane
     * See FollowCameraOptions.horizontalTranslationInertiaRate for full explanation
     */
    horizontalTranslationInertiaRate: HorizonProperty<number | null>;
    /**
     * How quickly the camera follows the target along Y axis plane
     * See FollowCameraOptions.verticalTranslationInertiaRate for full explanation
     */
     ...
     * If not set, the camera is always snapped to it instantly.
     *
     * @deprecated use verticalTranslationInertiaRate and horizontalTranslationInertiaRate instead
     */
    translationSpeed: HorizonProperty<number | null>;
    /**
     * Controls how quickly the camera rotates to the desired rotation.
